...
pub fn watch_library_folders(
    ...
    *guard = None; // drop (and stop) whatever was watched before
    ...
    let mut debouncer = new_debouncer(Duration::from_millis(400), move |result: notify_debouncer_mini::DebounceEventResult| {
        if result.is_ok() {
    ...
    for folder in &folders {
        ...
        if let Err(e) = debouncer.watcher().watch(Path::new(folder), RecursiveMode::NonRecursive) {
